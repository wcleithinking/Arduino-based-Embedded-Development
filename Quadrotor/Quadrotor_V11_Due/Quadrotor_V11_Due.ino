/*   **************************   Log   **********************
     ...
     Modified by Wenchao Lei, Nov. 13, 2016: add another PID parameters according to the PID tunner in MATLAB.
     Modified by Wenchao Lei, Nov. 14, 2016: modify the STR controller for yaw-dynamics, version 4 and add EEPROM function.
     Modified by Wenchao Lei, Nov. 14, 2016: change Receiver for Due!!!
     Modified by Wenchao Lei, Nov. 23, 2016: remove altitude function and add SD log function.
     Modified by Wenchao Lei, Nov. 24, 2016: replace "SD.h" with "SdFat.h" to increase the speed of SD card.
*/
...
void setup() {
  ...
  Receiver();
  Sensor();
  led_low();  // indicate config finished
  delay(2000);
  led_high();
  delay(2000);
#ifdef DEBUG
