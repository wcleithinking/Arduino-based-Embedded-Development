...
float yaw_state[5] = {hatyaw, gyro_z_bias, M_north, mag_x_bias, mag_y_bias};
float roll_P[2][2] = {100, 0, 0, 100};
float pitch_P[2][2] = {100, 0, 0, 100};
float yaw_P[5][5] = {50, 0, 0, 0, 0, 0, 50, 0, 0, 0, 0, 0, 50, 0, 0, 0, 0, 0, 50, 0, 0, 0, 0, 0, 50};
...
void kalman_filter_yaw() {
  float det, det_inv, prex[5], e[2];
  float preP[5][5], h[2][2], S[2][2], S_inv[2][2], temp[5][5], K[5][2];
  // predict
  prex[0] = yaw_state[0] + dt * (gyro_z - yaw_state[1]);
  for (int i = 1; i < 5; i++) prex[i] = yaw_state[i];
  preP[0][0] = yaw_P[0][0] - 2 * dt * yaw_P[0][1] + dt * dt * yaw_P[1][1] + dt * dt;
